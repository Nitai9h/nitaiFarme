...
async function validateJSUrl(url) {
  try {
    ...
    try {
      ...
      console.log('响应内容片段:', text.substring(0, 100));

      return response.ok;
    } catch (getError) {
      console.error('GET验证失败:', getError);
      return false;
    }
  }
}
...
function showInstallDialog(metadata, pluginUrl) {
  ...
  dialog.querySelector('.install-btn').addEventListener('click', async () => {
    try {
      // 调试日志
      console.log('开始安装:', { id: metadata.id, url: pluginUrl });
